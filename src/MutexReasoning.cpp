...
shared_ptr<Conflict> MutexReasoning::run(int a1, int a2, CBSNode& node,
                                         MDD* mdd_1, MDD* mdd_2)
{
    clock_t t = clock();
...
shared_ptr<Conflict> MutexReasoning::findMutexConflict(int a1, int a2,
                                                       CBSNode& node,
                                                       MDD* mdd_1, MDD* mdd_2)
{
    ...
    if (mutex_conflict == nullptr)
    {
        ...
        auto ip = IPMutexPropagation(&mdd_1_cpy, &mdd_2_cpy, search_engines[a1],
                                     search_engines[a2], ct1, ct2);
        con_vec a;
        con_vec b;
    ...
    // prepare for return
    shared_ptr<Conflict> conflict_to_return =
        make_shared<Conflict>(*mutex_conflict);

    if (swapped)
    {
        std::swap(conflict_to_return->a1, conflict_to_return->a2);
        std::swap(conflict_to_return->constraint1,
                  conflict_to_return->constraint2);
    }
