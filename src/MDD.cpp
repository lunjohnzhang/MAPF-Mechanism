...
/*bool MDD::isConstrained(int curr_id, int next_id, int next_timestep, const
std::vector< std::list< std::pair<int, int> > >& cons)  const
{
    if (cons.empty())
        return false;
    // check vertex constraints (being in next_id at next_timestep is
disallowed) if (next_timestep < static_cast<int>(cons.size()))
    {
        for (const auto & it : cons[next_timestep])
        {
            if ((std::get<0>(it) == next_id && std::get<1>(it) < 0)//vertex
constraint
                   ...
                   const SingleAgentSolver* _solver)
{
    ...
    while (!open.empty())
    {
        ...
        if (goal_node == nullptr &&
            curr->location ==
                solver->goal_location)  // arrive at the goal location
        {
            goal_node = curr;
                              ...
                              // edges in this step.
        {
            ...
            else  // If the child node exists
            {
                (*it)->parents.push_back(
                    curr);  // then add corresponding parent link and child link
                delete next;
            }
        }
    }
    ...
    while (!Q.empty())
    {
        ...
        for (auto parent : curr->parents)
        {
            if (curr == goal_node && parent->location == goal_node->location)
                continue;  // the parent of the goal node should not be at the
                           // goal location
            if (parent->mdd_node == nullptr)  // a new node
            {
                parent->mdd_node =
                    new MDDNode(parent->location, parent->timestep);
                levels[parent->timestep].push_back(parent->mdd_node);
                Q.push_back(parent);
            }
            parent->mdd_node->children.push_back(
                curr->mdd_node);  // add forward edge
            curr->mdd_node->parents.push_back(
                parent->mdd_node);  // add backward edge
        }
    }
                   ...
                   const SingleAgentSolver* _solver)
{
    ...
    while (!open.empty())
    {
                              ...
                              // edges in this step.
        {
            if (solver->my_heuristic[next_location] <= heuristicBound &&
                ...
                !ct.constrained(curr->location, next_location,
                                curr->level + 1))  // valid move
            {
                auto child = closed.rbegin();
                bool find = false;
                for (; child != closed.rend() &&
                       ((*child)->level == curr->level + 1);
                     ++child)
                {
                    if ((*child)->location ==
                        next_location)  // If the child node exists
                    {
                        (*child)->parents.push_back(
                            curr);  // then add corresponding parent link and
                                    // child link
                        find = true;
                        break;
                    }
    ...
    {
        if (parent->location ==
            goal_node->location)  // the parent of the goal node should not be
...
/*bool MDD::buildMDD(const std::vector <std::list< std::pair<int, int> > >&
constraints, int numOfLevels, int start_location, const int* moves_offset, const
std::vector<int>& my_heuristic, int map_size, int num_col)
{
    ...
    while (!open.empty())
    {
        ...
        // We want (g + 1)+h <= f = numOfLevels - 1, so h <= numOfLevels - g. -1
because it's the bound of the children. double heuristicBound = numOfLevels -
node->level - 2 + 0.001; for (int i = 0; i < 5; i++) // Try every possible move.
We only add backward edges in this step.
        {
...
corresponding parent link and child link find = true; break;
    ...
    for (int t = numOfLevels - 1; t > 0; t--)
    {
        ...
        {
            for (auto parent = (*it)->parents.begin(); parent !=
(*it)->parents.end(); parent++)
            {
                if ((*parent)->children.empty()) // a new node
                {
                    levels[t - 1].push_back(*parent);
...
void MDD::deleteNode(MDDNode* node)
{
    ...
    for (auto parent = node->parents.begin(); parent != node->parents.end();
         ++parent)
    {
...
MDD::MDD(const MDD& cpy)  // deep copy
{
    ...
    for (size_t t = 0; t < levels.size() - 1; t++)
    {
        for (auto node = levels[t].begin(); node != levels[t].end(); ++node)
        {
            ...
            for (list<MDDNode*>::const_iterator cpyChild =
                     cpyNode->children.begin();
                 cpyChild != cpyNode->children.end(); ++cpyChild)
            {
                MDDNode* child =
                    find((*cpyChild)->location, (*cpyChild)->level);
                if (child == nullptr)
                {
                     ...
                     SingleAgentSolver* solver)
{
    ...
    for (int l = 0; l < numOfLevels - 1; l++)
    {
        ...
        for (auto& it : levels[l])
        {
            ...
            // for (int i = 0; i < 5; i++) // Try every possible move. We only
            // add backward edges in this step.
            {
                ...
                if (solver->my_heuristic[newLoc] <= heuristicBound &&
                    ...
                    !ct.constrained(it->location, newLoc,
                                    it->level + 1))  // valid move
                {
    ...
    for (int l = oldHeight; l < numOfLevels; l++)
    {
        ...
        while (!bfs_q.empty())
        {
            ...
            for (auto parent_ptr : ptr->parents)
            {
                ...
                if (closed.find(parent_ptr) == closed.end() &&
                    parent_ptr->cost == 0)
                {
...
MDD* MDDTable::getMDD(HLNode& node, int id, size_t mdd_levels)
{
    ...
    if (got != lookupTable[c.a].end())
    {
        assert((node.getName() == "CBS Node" &&
                got->second->levels.size() == mdd_levels) ||
               (node.getName() == "ECBS Node" &&
                got->second->levels.size() <= mdd_levels));
        return got->second;
    }
    releaseMDDMemory(id);
...
{
    if (id < 0 || lookupTable.empty() ||
        (int)lookupTable[id].size() < max_num_of_mdds)
        return;
