...
std::pair<con_vec, con_vec> IPMutexPropagation::gen_constraints()
{
    ...
    while (!found_solution)
    {
        ...
        // cout << (!cp.feasible(init_len_0 + inc_len - 1, init_len_1 + inc_len
        // - 1)) << endl;
        ...
        if (!cp.feasible(init_len_0 + inc_len - 1, init_len_1 + inc_len - 1))
        {
            ...
            if (init_len_0 < init_len_1)
            {
                ...
                while (
                    init_len_0 + inc_0 <= init_len_1 + inc_1 &&
                                ...
                                init_len_1 + inc_1 + max(inc_len - 1, 0) - 1))
                {
                    ...
                    inc_0 += 1;
                    MDD_0->increaseBy(cons_0, 1, search_engine_0);
                    // cout <<  init_len_0 + inc_0 +  max(inc_len - 1, 0) - 1 <<
                    // " " << init_len_1 + inc_1 + max(inc_len - 1, 0) - 1 <<
                    // endl; cout << cp.feasible( init_len_0 + inc_0 +
                    // max(inc_len - 1, 0) - 1, init_len_1 + inc_1 + max(inc_len
                    // - 1, 0) - 1) << endl;
                }
                // cout << "inreasing a0 " << inc_0 << endl;
                // cout << init_len_0 + max(inc_0 - 1, 0) + max(inc_len - 1, 0)
                // - 1 << " " << init_len_1 + max(inc_len - 1, 0) - 1 << endl;
                return cp.generate_constraints(
                    init_len_0 + max(inc_0 - 1, 0) + max(inc_len - 1, 0) - 1,
