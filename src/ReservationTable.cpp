    ...
    for (auto it = sit[location].begin(); it != sit[location].end();)
    {
        ...
        else if (get<0>(*it) < t_min && t_max < get<1>(*it))
        {
            sit[location].insert(it,
                                 make_tuple(get<0>(*it), t_min, get<2>(*it)));
            (*it) = make_tuple(t_max, get<1>(*it), get<2>(*it));
            break;
        }
                                                ...
                                                int t_max)
{
    ...
    for (auto it = sit[location].begin(); it != sit[location].end(); ++it)
    {
        ...
        {
            if (it != sit[location].end() and
                std::next(it) != sit[location].end() and
                i_max == get<0>(*std::next(it)) and
                get<2>(*std::next(it)))  // we can merge the current interval
                                         // with the next one
            {
        ...
        else if (t_min <= i_min && t_max < i_max)
        {
            if (it != sit[location].begin() and
                ...
                get<2>(*std::prev(it)))  // we can merge the current interval
                                         // with the previous one
            {
                (*std::prev(it)) =
                    make_tuple(get<0>(*std::prev(it)), t_max, true);
            }
            else
            {
        ...
        else  // constraint_min <= get<0>(*it) && get<1> <= constraint_max
        {
                                         ...
                                         // with the previous one
            {
                if (it != sit[location].end() and
                    std::next(it) != sit[location].end() and
                    i_max == get<0>(*std::next(it)) and
                    get<2>(*std::next(it)))  // we can merge the current
                                             // interval with the next one
                {
                    (*std::prev(it)) = make_tuple(get<0>(*std::prev(it)),
                                                  get<1>(*std::next(it)), true);
                ...
                {
                    (*std::prev(it)) =
                        make_tuple(get<0>(*std::prev(it)), i_max, true);
                    it = sit[location].erase(it);
                }
                --it;
            }
    ...
    for (auto interval : sit[to])
    {
        ...
        {     // so we need to check the move action has collisions or not
            ...
            else
            {
                ...
                rst.emplace_back(get<1>(interval), t2, get<1>(interval), false,
                                 false);
            }
        }
    }
...
bool ReservationTable::find_safe_interval(Interval& interval, size_t location,
                                          int t_min)
{
...
int ReservationTable::get_earliest_arrival_time(int from, int to,
                                                int lower_bound,
...
int ReservationTable::get_earliest_no_collision_arrival_time(
    int from, int to, const Interval& interval, int lower_bound,
    int upper_bound) const
{
    for (auto t = max(lower_bound, get<0>(interval));
         t < min(upper_bound, get<1>(interval)); t++)
    {
