...
bool CBS::findPathForSingleAgent(CBSNode*  node, int ag, int lowerbound)
{
	...
	// find a path
	Path new_path = search_engines[ag]->findOptimalPath(*node, initial_constraints[ag], paths, ag, lowerbound, dummy_start_node);
...
bool CBS::generateRoot()
{
	...
	if (paths_found_initially.empty())
	{
		...
		for (auto i : agents)
		{
			...
			paths_found_initially[i] = search_engines[i]->findOptimalPath(*root, initial_constraints[i], paths, i, 0, dummy_start_node);
