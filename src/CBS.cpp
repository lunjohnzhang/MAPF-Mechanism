...
void CBS::findConflicts(HLNode& curr)
{
    ...
    // curr.distance_to_go = (int)(curr.unknownConf.size() +
    // curr.conflicts.size());
...
void CBS::computeSecondPriorityForConflict(Conflict& conflict,
                                           const HLNode& node)
{
    int count[2] = {0, 0};
...
bool CBS::findPathForSingleAgent(CBSNode* node, int ag, int lowerbound)
{
    ...
    // find a path
    Path new_path = search_engines[ag]->findOptimalPath(
    ...
    else if (type == constraint_type::POSITIVE_VERTEX)
    {
        ...
        for (const auto& constraint : node->constraints)
        {
            ...
            for (int ag = 0; ag < num_of_agents; ag++)
            {
                ...
                {
                    if (!findPathForSingleAgent(node, ag, (int)paths[ag]->size()
    - 1))
                    {
                        runtime_generate_child += (double)(clock() - t1) /
    CLOCKS_PER_SEC; return false;
...
inline void CBS::pushNode(CBSNode* node)
{
    ...
    case high_level_solver_type::ASTAR:
        break;
    case high_level_solver_type::ASTAREPS:  // cleanup_list is called open_list
                                            // in ECBS
...
CBSNode* CBS::selectNode()
{
    ...
    case high_level_solver_type::ASTAREPS:
        ...
        if (cleanup_list.top()->getFVal() > cost_lowerbound)
        {
            ...
            {
                if (n->getFVal() > old_focal_list_threshold &&
                    n->getFVal() <= new_focal_list_threshold)
    ...
    case high_level_solver_type::EES:
        ...
        if (open_list.top()->getFHatVal() > inadmissible_cost_lowerbound)
        {
            ...
            {
                cout << "  Note -- FOCAL UPDATE!! from |FOCAL|="
                     << focal_list.size() << " with |OPEN|=" << open_list.size()
                     << " to |FOCAL|=";
            }
...
void CBS::saveCT(const string& fileName) const  // write the CT to a file
{
    // Write the tree graph in dot language to a file
    {
        ...
        {
            output << node->time_generated << " [label=\"g=" << node->g_val
                   << ", h=" << node->h_val << "\nd=" << node->distance_to_go
                   << ", h^=" << node->getFHatVal() - node->g_val;
            if (node->time_expanded > 0)  // the node has been expanded
            {
                output << "\n #" << node->time_expanded << " from "
                       << node->chosen_from;
                output << "\", color=";
    ...
    // Write the stats of the tree to a CSV file
    {
        ...
        {
            output << node->time_generated << "," << node->g_val << ","
                   << node->h_val << "," << node->getFHatVal() - node->g_val
                   << "," << node->distance_to_go << "," << node->depth << ","
                   << node->time_expanded << "," << node->chosen_from << ","
                   << node->h_computed << "," << node->f_of_best_in_cleanup
                   << "," << node->f_hat_of_best_in_cleanup << ","
                   << node->d_of_best_in_cleanup << ","
                   << node->f_of_best_in_open << ","
                   << node->f_hat_of_best_in_open << ","
                   << node->d_of_best_in_open << "," << node->f_of_best_in_focal
                   << "," << node->f_hat_of_best_in_focal << ","
...
bool CBS::solve(double _time_limit, int _cost_lowerbound, int _cost_upperbound)
{
    ...
    while (!cleanup_list.empty() && !solution_found)
    {
        ...
        if (!curr->h_computed)  // heuristics has not been computed yet
        {
            ...
            heuristic_helper.updateInadmissibleHeuristics(
                *curr);  // compute inadmissible heuristics
...
void CBS::addConstraints(const HLNode* curr, HLNode* child1,
                         HLNode* child2) const
{
    if (disjoint_splitting && curr->conflict->type == conflict_type::STANDARD)
    {
        ...
        if (first)  // disjoint splitting on the first agent
        {
            ...
            {
                child2->constraints.emplace_back(
        ...
        else  // disjoint splitting on the second agent
        {
            ...
            if (type == constraint_type::VERTEX)
            {
                child1->constraints.emplace_back(
                    a, x, y, t, constraint_type::POSITIVE_VERTEX);
            }
            else
            {
    ...
    if (paths_found_initially.empty())
    {
        ...
        for (auto i : agents)
        {
            ...
            paths_found_initially[i] = search_engines[i]->findOptimalPath(
                *root, initial_constraints[i], paths, i, 0, dummy_start_node);
