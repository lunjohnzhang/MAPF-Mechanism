...
bool ConstraintPropagation::should_be_bwd_mutexed(MDDNode* node_a,
                                                  MDDNode* node_b)
{
...
bool ConstraintPropagation::should_be_fwd_mutexed(MDDNode* node_a,
                                                  MDDNode* node_b)
{
...
void ConstraintPropagation::add_fwd_edge_mutex(MDDNode* node_a,
                                               MDDNode* node_a_to,
                                               MDDNode* node_b,
                                               MDDNode* node_b_to)
{
        ...
        for (auto& node_0 : mdd0->levels[i])
        {
            ...
            for (auto node_0_to : node_0->children)
            {
                ...
                for (auto ptr : node_1->children)
                {
                    if (ptr == node_1_to)
                    {
                        add_fwd_edge_mutex(node_0, node_0_to, node_1,
                                           node_1_to);
                    }
                }
            }
        ...
        else
        {
            ...
            for (auto node_a_ch : node_a->children)
            {
                for (auto node_b_ch : node_b->children)
                {
                    ...
                    auto new_mutex =
                        std::make_pair(std::make_pair(node_a_ch, nullptr),
...
void ConstraintPropagation::bwd_mutex_prop()
{
    ...
    while (!open.empty())
    {
        ...
        if (is_edge_mutex(mutex))
        {
            ...
            auto new_mutex =
                std::make_pair(std::make_pair(node_from_1, nullptr),
        ...
        else
        {
            ...
            for (auto node_a_pa : node_a->parents)
            {
                for (auto node_b_pa : node_b->parents)
                {
                    ...
                    auto new_mutex =
                        std::make_pair(std::make_pair(node_a_pa, nullptr),
...
ConstraintPropagation::generate_constraints(int level_0, int level_1)
{
    ...
    if (!non_mutexed.empty())
    {
        ...
        // boost::unordered_set<MDDNode*> level_j(non_mutexed.begin(),
        // non_mutexed.end());
        ...
        Constraint length_con(0, goal_ptr_i->location, -1, level_0 - 1,
        ...
        {
            cons_vec_1.push_back(Constraint(1, it.second, -1, it.first,
                                            constraint_type::VERTEX));
        }
        // std::vector<std::pair<int, int>> cons_vec_1(cons_set_1.begin(),
        // cons_set_1.end());
        cons_vec_1.push_back(Constraint(0, goal_ptr_i->location, -1,
                                        level_0 - 1,
    ...
    for (auto& it : cons_0)
    {
        cons_vec_0.push_back(
            {0, it->location, -1, it->level, constraint_type::VERTEX});
    }
    for (auto& it : cons_1)
    {
        cons_vec_1.push_back(
            {1, it->location, -1, it->level, constraint_type::VERTEX});
    }
    if (reversed)
    {
