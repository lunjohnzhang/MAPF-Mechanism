...
void CBSHeuristic::updateInadmissibleHeuristics(HLNode& curr)
{
    ...
    case heuristics_type::GLOBAL:  // Note: there is no "break" in the previous
                                   // line, so here we compute heuristics for
        ...
        {
            c = sum_cost_errors[0] /
                (num_of_errors[0] - sum_distance_errors[0]);
        }
    ...
    case heuristics_type::LOCAL:
        if (std::abs(1 - sum_distance_errors[0]) < 0.001)
            ...
            curr.cost_to_go =
                h + max(0, (int)(curr.distance_to_go * sum_cost_errors[0] /
                                 (1 - sum_distance_errors[0])));
    ...
    case heuristics_type::CONFLICT:
        ...
        max(conflict->a1, conflict->a2)); if (conflicting_agents.find(agents) !=
        conflicting_agents.end()) continue; // we do not recount the conflict
        between the same pair of agents int id = conflict->getConflictId();
            cost_error += getCostError(id);
            distance_error += getDistanceError(id);
        }*/
        cost_error /=
            (double)(curr.conflicts.size());  // + conflicting_agents.size());
        distance_error /=
            (double)(curr.conflicts.size());  // + conflicting_agents.size());
        ...
        curr.parent != nullptr)
    {
        ...
        for (auto child : curr.parent->children)
        {
            ...
            if (best->getFVal() > child->getFVal() ||
                (best->getFVal() == child->getFVal() &&
                 best->distance_to_go > child->distance_to_go))
                best = child;
        }
        if (curr.parent->fully_expanded)  // update error
        {
            curr.parent->distance_error =
                1 + best->distance_to_go - curr.parent->distance_to_go;
            curr.parent->cost_error = best->g_val + best->h_val -
                                      curr.parent->g_val - curr.parent->h_val;
            if (inadmissible_heuristic ==
                heuristics_type::GLOBAL)  // update global error
            {
                sum_distance_errors[0] += curr.parent->distance_error;
                sum_cost_errors[0] += curr.parent->cost_error;
                num_of_errors[0]++;
            }
            else if (inadmissible_heuristic ==
                     heuristics_type::LOCAL)  // update local error
            {
                num_of_errors[0]++;
                ...
                sum_distance_errors[0] =
                    sum_distance_errors[0] * (1 - learning_rate) +
                    curr.parent->distance_error * learning_rate;
                sum_cost_errors[0] = sum_cost_errors[0] * (1 - learning_rate) +
                                     curr.parent->cost_error * learning_rate;
            }
            else if (inadmissible_heuristic ==
                     heuristics_type::CONFLICT)  // update conflict error
            {
...
void CBSHeuristic::updateOnlineHeuristicErrors(ECBSNode& parent)
{
    ...
    // Update the errors
    parent.distance_error =
        1 + best_child->distance_to_go - parent.distance_to_go;
    parent.cost_error =
        best_child->getFHatVal() - best_child->cost_to_go - parent.sum_of_costs;
    if (inadmissible_heuristic ==
        heuristics_type::GLOBAL)  // update global error
    {
             ...
             heuristics_type::LOCAL)  // update local error
    {
        ...
        sum_distance_errors[0] = sum_distance_errors[0] * (1 - learning_rate) +
                                 parent.distance_error * learning_rate;
        sum_cost_errors[0] = sum_cost_errors[0] * (1 - learning_rate) +
                             parent.cost_error * learning_rate;
    }
    else if (inadmissible_heuristic ==
             heuristics_type::CONFLICT)  // update conflict error
    {
...
void CBSHeuristic::computeQuickHeuristics(HLNode& node)
{
    ...
    {
    if ((e.first / num_of_agents == agent || e.first % num_of_agents == agent)
    && e.second > maxWeight)
    {
    maxWeight = e.second;
...
bool CBSHeuristic::computeInformedHeuristics(CBSNode& curr, double _time_limit)
{
    ...
    case heuristics_type::CG:
        ...
        if (curr.parent == nullptr ||
            num_of_CGedges > ILP_edge_threshold ||  // root node of CBS tree or
                                                    // the graph is too large
            target_reasoning ||
            disjoint_splitting)  // when we are allowed to replan for multiple
                                 // agents, the incremental method is not
                                 // correct any longer.
            h = minimumVertexCover(HG);
        else
            h = minimumVertexCover(HG, curr.parent->h_val, num_of_agents,
                                   num_of_CGedges);
        break;
...
bool CBSHeuristic::computeInformedHeuristics(ECBSNode& curr,
                                             ...
                                             double _time_limit)
{
    ...
    root node of CBS tree or the graph is too large h = minimumVertexCover(HG);
        else
            h = minimumVertexCover(HG, curr.parent->h_val, num_of_agents,
    num_of_CGedges); break; case heuristics_type::DG: if
...
j]); if (!done[k])
        ...
        {
            rst += std::max(HG[indices[0] * num_of_agents + indices[1]],
HG[indices[1] * num_of_agents + indices[0]]); // add edge weight continue;
        }
...
void CBSHeuristic::buildCardinalConflictGraph(CBSNode& curr, vector<int>& CG,
                                              int& num_of_CGedges)
{
    num_of_CGedges = 0;
    for (const auto& conflict : curr.conflicts)
...
bool CBSHeuristic::buildDependenceGraph(CBSNode& node, vector<int>& CG,
                                        int& num_of_CGedges)
{
    num_of_CGedges = 0;
    for (auto& conflict : node.conflicts)
    {
        ...
        {
            num_of_CGedges++;
            conflict->priority = conflict_priority::
                PSEUDO_CARDINAL;  // the two agents are dependent, although
                                  // resolving this conflict might not increase
                                  // the cost
        }
    }
    runtime_build_dependency_graph +=
...
bool CBSHeuristic::buildWeightedDependencyGraph(CBSNode& node, vector<int>& CG)
{
    for (const auto& conflict : node.conflicts)
    {
        ...
        else
        {
            ...
            if (!cardinal &&
                !mutex_reasoning)  // using merging MDD methods before runing
                                   // 2-agent instance
            {
...
bool CBSHeuristic::buildWeightedDependencyGraph(ECBSNode& node,
                                                const vector<int>& min_f_vals,
...
pair<int, int> CBSHeuristic::solve2Agents(int a1, int a2, const CBSNode& node,
                                          bool cardinal)
{
    ...
    double runtime = (double)(clock() - start_time) / CLOCKS_PER_SEC;
    int root_g =
        (int)initial_paths[0].size() - 1 + (int)initial_paths[1].size() - 1;
    ...
    if (cbs.runtime >= time_limit - runtime ||
        ...
        rst.first =
            cbs.getLowerBound() - root_g;  // using lowerbound to approximate
    ...
    if (save_stats)
    {
        sub_instances.emplace_back(a1, a2, &node, cbs.num_HL_expanded,
                                   rst.second);
    }
    return rst;
}
...
tuple<int, int, int> CBSHeuristic::solve2Agents(int a1, int a2,
                                                const ECBSNode& node)
{
    vector<SingleAgentSolver*> engines{search_engines[a1], search_engines[a2]};
    ...
    if (cbs.runtime >= time_limit - runtime)  // time out
        ...
        return make_tuple(
            cbs.getLowerBound() - cbs.dummy_start->g_val,
            cbs.getInitialPathLength(0),
            cbs.getInitialPathLength(1));  // using lowerbound to approximate
    else if (cbs.solution_cost < 0)        // no solution
        return make_tuple(MAX_COST, cbs.getInitialPathLength(0),
...
int CBSHeuristic::getEdgeWeight(int a1, int a2, CBSNode& node, bool cardinal)
{
    ...
    if (screen > 2)
    {
        cout << "Agents " << a1 << " and " << a2 << " in node " <<
node.time_generated << " : ";
    }
    int rst = 0;
    if (cardinal)
        rst = 1;
    else if (!mutex_reasoning && // no mutex reasoning, so we might miss some
cardinal conflicts (type == heuristics_type::DG || type ==
heuristics_type::WDG))
    {
        // get mdds
    ...
    if (type == heuristics_type::WDG && rst > 0)
    {
        ...
        int upperbound = (int)initial_paths[0].size() +
(int)initial_paths[1].size() + 10; vector<ConstraintTable> constraints{
            ConstraintTable(initial_constraints[a1]),
            ConstraintTable(initial_constraints[a2]) };
        ...
        cbs.setDisjointSplitting(disjoint_splitting);
        cbs.setBypass(false); // I guess that bypassing does not help two-agent
path finding??? cbs.setRectangleReasoning(rectangle_reasoning);
        cbs.setCorridorReasoning(corridor_reasoning);
        cbs.setTargetReasoning(target_reasoning);
        cbs.setMutexReasoning(mutex_reasoning);
...
approximate else if (cbs.solution_cost  < 0) // no solution rst =
cbs.solution_cost; else rst = cbs.solution_cost - cost_shortestPath;
        num_solve_2agent_problems++;
    }
...
finding??? cbs.setRectangleReasoning(rectangle_reasoning);
    ...
    cbs.setConflictSelectionRule(conflict_selection_rule);
    cbs.setNodeSelectionRule(node_selection_rule);
    cbs.setHighLevelSolver(high_level_solver_type::ASTAR, 1); // solve the sub
problem optimally cbs.setNodeLimit(node_limit);
}*/
...
int CBSHeuristic::minimumVertexCover(const vector<int>& CG)
{
    ...
    for (int i = 0; i < num_of_agents; i++)
    {
        ...
        if ((int)indices.size() == 1)  // one node -> no edges -> mvc = 0
            continue;
        else if ((int)indices.size() ==
                 2)  // two nodes -> only one edge -> mvc = 1
        {
        ...
        for (int j = 0; j < (int)indices.size(); j++)
        {
            ...
            {
                subgraph[j * indices.size() + k] =
                    CG[indices[j] * num_of_agents + indices[k]];
                subgraph[k * indices.size() + j] =
        ...
        else
        {
            ...
            {
                if (KVertexCover(subgraph, (int)indices.size(), num_edges,
                                 i = k, (int)indices.size()))
                {
                    rst += k;
                    break;
                }
...
int CBSHeuristic::minimumVertexCover(const std::vector<int>& CG, int old_mvc,
                                     int cols, int num_of_CGedges)
{
    clock_t t = clock();
    int rst = 0;
    if (num_of_CGedges < 2)
    ...
    else
    {
        if (KVertexCover(CG, num_of_CGnodes, num_of_CGedges, old_mvc - 1, cols))
            rst = old_mvc - 1;
        else if (KVertexCover(CG, num_of_CGnodes, num_of_CGedges, old_mvc,
                              cols))
            rst = old_mvc;
        else
            rst = old_mvc + 1;
...
bool CBSHeuristic::KVertexCover(const std::vector<int>& CG, int num_of_CGnodes,
                                int num_of_CGedges, int k, int cols)
{
    double runtime = (double)(clock() - start_time) / CLOCKS_PER_SEC;
    if (runtime > time_limit)
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        if (KVertexCover(CG_copy, num_of_CGnodes - 1, num_of_CGedges_copy,
                         k - 1, cols))
            return true;
    }
    return false;
    ...
    for (int i = 0; i < num_of_agents; i++)
    {
        ...
        while (!Q.empty())
        {
            ...
            for (int k = 0; k < num_of_agents; k++)
            {
                ...
                {
                    range[num] =
                        std::max(range[num], CG[j * num_of_agents + k]);
                    if (!done[k])
                    {
                        Q.push(k);
                        done[k] = true;
                ...
                {
                    range[num] =
                        std::max(range[num], CG[k * num_of_agents + j]);
                    if (!done[k])
                    {
                        Q.push(k);
                        done[k] = true;
        ...
        else if (num == 2)  // only one edge
        {
            rst += std::max(CG[indices[0] * num_of_agents + indices[1]],
                            CG[indices[1] * num_of_agents +
                               indices[0]]);  // add edge weight
            continue;
        }
        std::vector<int> G(num * num, 0);
        for (int j = 0; j < num; j++)
        {
            ...
            {
                G[j * num + k] =
                    std::max(CG[indices[j] * num_of_agents + indices[k]],
                             CG[indices[k] * num_of_agents + indices[j]]);
            }
        }
        if (num > ILP_node_threshold)
...
int CBSHeuristic::DPForWMVC(std::vector<int>& x, int i, int sum,
                            ...
                            const std::vector<int>& range, int& best_so_far)
{
    ...
    for (int j = 0; j < i; j++)
    {
        ...
        {
            min_cost = CG[j * cols + i] -
                       x[j];  // cost should be at least CG[i][j] - x[j];
        }
    }
...
/*int CBSHeuristic::ILPForWMVC(const vector<int>& CG, const vector<int>&
node_max_value) const
{
...
/*int CBSHeuristic::ILPForConstrainedWMVC(const std::vector<int>& CG, const
std::vector<int>& node_weights)
{
    ...
    IloCplex cplex(env);
    double runtime = (double)(clock() - start_time) / CLOCKS_PER_SEC;
    cplex.setParam(IloCplex::TiLim, time_limit - runtime); // time limit = 300
sec int solution_cost = -1; cplex.extract(model);
    cplex.setOut(env.getNullStream());
    int rst = 0;
    if (cplex.solve())
        rst = (int)cplex.getObjValue();
                                       ...
                                       int& best_so_far)
{
    ...
    for (int j = 0; j < i; j++)
    {
        if (min_cost + x[j] * range[j] <
            CG[j * cols + i])  // infeasible assignment
        {
            min_cost =
                CG[j * cols + i] -
                x[j] * range[j];  // cost should be at least CG[i][j] - x[j];
        }
    }
    if (min_cost == 0)
    {
    ...
    {
        x[i] = 1;
        int rst = DPForConstrainedWMVC(x, i + 1, sum + x[i] * range[i], CG,
                                       range, best_so_far);
        if (rst < best_so_far)
        {
            best_so_far = rst;
...
bool CBSHeuristic::dependent(
    int a1, int a2,
    HLNode& node)  // return true if the two agents are dependent
{
    const MDD* mdd1 =
        mdd_helper.getMDD(node, a1, paths[a1]->size());  // get mdds
    const MDD* mdd2 = mdd_helper.getMDD(node, a2, paths[a2]->size());
    if (mdd1->levels.size() > mdd2->levels.size())  // swap
        std::swap(mdd1, mdd2);
    num_merge_MDDs++;
...
bool CBSHeuristic::SyncMDDs(
    const MDD& mdd, const MDD& other)  // assume mdd.levels <= other.levels
{
    if (other.levels.size() <=
        1)  // Either of the MDDs was already completely pruned already
        return false;
    ...
    copy.levels[0].front()->coexistingNodesFromOtherMdds.push_back(
        other.levels[0].front());
    ...
    for (size_t i = 1; i < copy.levels.size(); i++)
    {
        ...
        {
            // Go over all the node's parents and test their coexisting nodes'
            // children for co-existance with this node
            for (auto parent = (*node)->parents.begin();
                 parent != (*node)->parents.end(); parent++)
            {
                // bool validParent = false;
                for (const MDDNode* parentCoexistingNode :
                     (*parent)->coexistingNodesFromOtherMdds)
                {
                    for (const MDDNode* childOfParentCoexistingNode :
                         parentCoexistingNode->children)
                    {
                        if ((*node)->location ==
                                childOfParentCoexistingNode
                                    ->location ||  // vertex conflict
                            ((*node)->location ==
                                 parentCoexistingNode->location &&
                             (*parent)->location ==
                                 childOfParentCoexistingNode
                                     ->location))  // edge conflict
                            continue;

                        auto it =
                            (*node)->coexistingNodesFromOtherMdds.cbegin();
                        for (;
                             it != (*node)->coexistingNodesFromOtherMdds.cend();
                             ++it)
                        {
                        ...
                        {
                            (*node)->coexistingNodesFromOtherMdds.push_back(
                                childOfParentCoexistingNode);
                        }
                    }
                }
