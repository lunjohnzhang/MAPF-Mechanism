...
Path SIPP::findPath(const ConstraintTable& constraint_table)
{
    ...
    auto h = max(my_heuristic[start_location], last_target_collision_time + 1);
    auto start =
    ...
    double agent_w, bool dummy_start_node)
{
    ...
    while (!open_list.empty())
    {
                ...
                reservation_table.constraint_table.length_max)
        {
            auto next_timestep = get<0>(interval);
            int next_h_val =
                max(curr->h_val, curr->getFVal() - next_timestep);  // path max
            auto next_collisions =
                curr->num_of_conflicts +
                (int)curr->collision_v *
                    max(next_timestep - curr->timestep - 1, 0)  // wait time
                        ...
                        int upper_bound)
{
    ...
    // boost::heap::pairing_heap< AStarNode*,
    boost::heap::compare<LLNode::compare_node> >::handle_type open_handle;
    ...
    auto root = new SIPPNode(start, 0, instance.getManhattanDistance(start,
    end), nullptr, 0, interval); root->open_handle = open_list.push(root);  //
    add root to heap nodes.insert(root);       // add root to hash_table (nodes)
    ...
    while (!open_list.empty())
    {
        ...
        {
            if ((curr->location == blocked.first && next_location ==
    blocked.second) || (curr->location == blocked.second && next_location ==
    ...
    get<1>(curr->interval) + 1))
            {
                int next_timestep = max(curr->timestep + 1,
    (int)get<0>(interval)); int next_g_val = next_timestep; int next_h_val =
    instance.getManhattanDistance(next_location, end); if (next_g_val +
    next_h_val >= upper_bound) // the cost of the path is larger than the upper
    bound continue; auto next = new SIPPNode(next_location, next_g_val,
    next_h_val, nullptr, next_timestep, interval); auto it = nodes.find(next);
                if (it == nodes.end())
    ...
    heap) delete(next);  // not needed anymore -- we already generated it before
...
bool SIPP::dominanceCheck(SIPPNode* new_node)
{
    ...
    for (auto& old_node : ptr->second)
    {
        ...
        {  // intervals overlap --> we need to split the node to make them
           // disjoint
