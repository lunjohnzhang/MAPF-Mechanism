...
public:
    ...
    struct compare_node_by_f
    {
        bool operator()(const ECBSNode* n1, const ECBSNode* n2) const
        {
            if (n1->g_val + n1->h_val == n2->g_val + n2->h_val)
            {
                ...
                {
                    if (n1->sum_of_costs + n1->cost_to_go ==
                        n2->sum_of_costs + n2->cost_to_go)
                    {
                        return n1->h_val >= n2->h_val;
                    }
                    return n1->sum_of_costs + n1->cost_to_go >=
                           n2->sum_of_costs + n2->cost_to_go;
                }
    ...
    };  // used by FOCAL to compare nodes by f^-val (top of the heap has min
        // f^-val)

    pairing_heap<ECBSNode*, compare<ECBSNode::compare_node_by_f> >::handle_type
        cleanup_handle;
    pairing_heap<ECBSNode*,
                 compare<ECBSNode::compare_node_by_inadmissible_f> >::
