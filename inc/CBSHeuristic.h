...
struct HTableEntry  // look-up table entry
{
    ...
    struct EqNode
    {
        bool operator()(const HTableEntry& h1, const HTableEntry& h2) const
        {
            ...
            {
                if (get<4>(curr->constraints.front()) ==
            ...
            if (cons1[0].size() != cons1[1].size() ||
                cons2[0].size() != cons2[1].size())
                return false;
...
typedef unordered_map<HTableEntry, tuple<int, int, int>, HTableEntry::Hasher,
                      HTableEntry::EqNode>
    HTable;
...
public:
    ...
    list<tuple<int, int, const HLNode*, uint64_t, int> >
        sub_instances;  // <agent 1, agent 2, node, number of expanded CT nodes,
                        // h value>
    ...
    void setInadmissibleHeuristics(heuristics_type h)
    {
        ...
        if (h == heuristics_type::CONFLICT)
        {
            sum_distance_errors.assign(
                ...
                0);  //(int(conflict_priority::PRIORITY_COUNT) *
                     //int(conflict_type::TYPE_COUNT), 0);
    ...
    {
        return (num_of_errors[i] == 0) ? 0
                                       : sum_cost_errors[i] / num_of_errors[i];
    }
    double getDistanceError(int i = 0) const
    {
        return (num_of_errors[i] == 0)
                   ? 0
...
private:
    ...
    double time_limit;
    int node_limit = 4;  // terminate the sub CBS solver if the number of its
                         // expanded nodes exceeds the node limit.
    double start_time;
    int ILP_node_threshold = 5;    // when #nodes >= ILP_node_threshold, use ILP
                                   // solver; otherwise, use DP solver
    int ILP_edge_threshold = 10;   // when #edges >= ILP_edge_threshold, use ILP
                                   // solver; otherwise, use DP solver
    int ILP_value_threshold = 32;  // when value >= ILP_value_threshold, use ILP
                                   // solver; otherwise, use DP solver
    ...
    int DPForWMVC(vector<int>& x, int i, int sum, const vector<int>& CG,
                  const vector<int>& range,
                  int& best_so_far);  // dynamic programming
    // int ILPForWMVC(const vector<int>& CG, const vector<int>& range) const; //
    // Integer linear programming
    int ILPForConstrainedWMVC(const std::vector<int>& CG,
                              const std::vector<int>& range);
    int DPForConstrainedWMVC(vector<bool>& x, int i, int sum,
                             const vector<int>& CG, const vector<int>& range,
                             int& best_so_far);
};
