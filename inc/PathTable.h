...
public:
    int makespan = 0;
    vector<vector<int> > table;  // this stores the collision-free paths, the
                                 // value is the id of the agent
    ...
    int getHoldingTime(
        int goal_location,
        int earliest_timestep);  // Return the earliest timestep when the agent
                                 // can hold its goal location.
    void get_agents(set<int>& conflicting_agents, int loc) const;
    void get_agents(set<int>& conflicting_agents, int neighbor_size,
                    int loc) const;
    void getConflictingAgents(int agent_id, set<int>& conflicting_agents,
