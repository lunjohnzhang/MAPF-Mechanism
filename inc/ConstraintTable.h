...
public:
    ...
    void insert2CT(const HLNode& node,
                   int agent);  // build the constraint table for the given
                                // agent at the give node
    ...
    void insert2CAT(
        int agent,
        const vector<Path*>&
            paths);  // build the conflict avoidance table using a set of paths
    void insert2CAT(
        const Path& path);  // insert a path to the collision avoidance table
    // int getCATMaxTimestep() const {return cat_max_timestep;}

protected:
    friend class ReservationTable;
    typedef unordered_map<size_t, list<pair<int, int> > >
        CT;  // constraint table
    ...
    map<int, size_t> landmarks;  // <timestep, location>: the agent must be at
                                 // the given location at the given timestep
