...
private:
    ...
    shared_ptr<Conflict> findCorridorConflict(
        const shared_ptr<Conflict>& conflict, const vector<Path*>& paths,
        const HLNode& node);
    int findCorridor(
        const shared_ptr<Conflict>& conflict, const vector<Path*>& paths,
        int endpoints[],
        int endpoints_time[]);  // return the length of the corridor
    int getEnteringTime(const std::vector<PathEntry>& path,
                        const std::vector<PathEntry>& path2, int t);
    int getExitingTime(const std::vector<PathEntry>& path, int t);
    int getCorridorLength(const std::vector<PathEntry>& path, int t_start,
                          int loc_end, std::pair<int, int>& edge);

    // int getBypassLength(int start, int end, std::pair<int, int> blocked,
    // const bool* my_map, int num_col, int map_size); int
    // getBypassLengthByAStar(int start, int end, std::pair<int, int> blocked,
    //	const ConstraintTable& constraint_table, int upper_bound);
    // int getBypassLengthBySIPP(int start, int end, std::pair<int, int>
    // blocked,
